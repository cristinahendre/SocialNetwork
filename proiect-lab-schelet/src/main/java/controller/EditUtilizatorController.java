...
public class EditUtilizatorController {
    ...
    private Node createPage(int pageIndex) {
        ...
        Page<Utilizator> users =service.getFriendsOnPage(pageIndex,curent.getId());
    ...
    public void handleDeleteFriend(ActionEvent event) {
        ...
        try {
            Utilizator[] users= tableView.getSelectionModel().getSelectedItems().toArray(new Utilizator[0]);
            if(users.length==0){
                MessageAlert.showErrorMessage(null,"Selectati un utilizator.");
                return;
            }
            for (Utilizator u : users) {

                prietenieService.stergePrietenie(curent.getId(), u.getId());
            }
            initModel();
        }
    ...
    public void addFriends(ActionEvent event) {
        try {
            ...
            loader.setLocation(getClass().getResource("/views/fereastraPrieteni.fxml"));

            AnchorPane root =  loader.load();

            // Create the dialog Stage.
            Stage dialogStage = new Stage();
            dialogStage.setTitle("Adauga un Prieten");
            ...
            AdaugaPrietenController ctr = loader.getController();
            ctr.setService(service, dialogStage, curent,prietenieService,cerereDePrietenie);
